...
def evaluate_sequence_reg(model, cfg):
    ...
    revisit_json_file = 'is_revisit_D-{}_T-{}.json'.format(int(cfg.revisit_criteria), int(cfg.skip_time))
    if 'Kitti' in cfg.eval_dataset:
        ...
        _, positions_database = load_poses_from_csv(sequence_path + '/scan_poses.csv')
        timestamps = load_timestamps_csv(sequence_path + '/scan_poses.csv')
        revisit_json_dir = os.path.join(os.path.dirname(__file__), '../config/mulran_tuples/')
        revisit_json = json.load(open(revisit_json_dir + revisit_json_file, "r"))
        is_revisit_list = revisit_json[eval_seq]

    logging.info(f'Evaluating sequence {eval_seq} at {sequence_path}')
    thresholds = np.linspace(cfg.cd_thresh_min, cfg.cd_thresh_max, int(cfg.num_thresholds))
    ...
    for query_idx in range(num_queries):
        ...
        tt = next(x[0] for x in enumerate(timestamps) if x[1] > (query_time - cfg.skip_time))
        ...
        logging.info(f'id: {query_idx} n_id: {nearest_idx} is_rev: {is_revisit} is_correct_loc: {is_correct_loc} min_dist: {min_dist} p_dist: {p_dist}')
    ...
    if save_counts:
        save_dir = os.path.join(os.path.dirname(__file__), 'pickles/', str(eval_seq))
        if not os.path.exists(save_dir): os.makedirs(save_dir)
