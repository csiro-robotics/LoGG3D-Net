...
def main():
  ...
  # Get model
  backbone_model_dir = os.path.join(os.path.dirname(__file__), 'pretrained')
  ...
  # Write train_one_epoch function
  if cfg.resume_training:
    resume_filename = cfg.resume_checkpoint
    save_path = os.path.join(os.path.dirname(__file__)  , 'checkpoints', resume_filename)
  ...
  for epoch in range(starting_epoch, cfg.max_epoch):
    ...
    for i, batch in enumerate(train_loader, 0):
      ...
      if cfg.point_loss_weight > 0:
        point_loss = point_loss_function(output[1][0], output[1][1], batch[1]['pos_pairs'], cfg)
      ...
      if (i % cfg.loss_log_step) == (cfg.loss_log_step - 1):
        ...
        if (dist.rank() % dist.size() == 0):
          lr = scheduler.get_last_lr()
          logging.info('avg running loss: '+ str(avg_loss) + ' LR: %03f' %(lr[0]))
          logging.info('avg_scene_loss: '+ str(avg_scene_loss) + ' avg_point_loss: '+ str(avg_point_loss))
