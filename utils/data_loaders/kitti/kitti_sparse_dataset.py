...
class KittiSparseTupleDataset(KittiTupleDataset):
  ...
  def __init__(self,
    ...
    KittiTupleDataset.__init__(self, phase, random_rotation, random_occlusion, random_scale, config)
...
class KittiPointSparseTupleDataset(KittiSparseTupleDataset):
  ...
  def __init__(self,
    ...
    KittiSparseTupleDataset.__init__(self, phase, random_rotation, random_occlusion, random_scale, config)
  ...
  def get_sparse_pcd(self, drive_id, pc_id):
    ...
    # xyz_range = np.linalg.norm(xyzr[:,:3], axis=1) < 60
    # xyzr = xyzr[xyz_range]
  ...
  def get_point_tuples(self, drive_id, query_id, pos_id):
    ...
    all_odometry = self.get_video_odometry(drive_id, [query_id, pos_id])
  ...
  def __getitem__(self, idx):
    ...
    try:
      ...
      logging.info(f"Drive id:{drive_id}, query id:{query_id}, ppq:{self.positives_per_query}, num_pos:{len(positive_ids)}")
      sel_positive_ids = random.choices(positive_ids, k=self.positives_per_query)
