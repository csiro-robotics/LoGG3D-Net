...
class PointCloudDataset(torch.utils.data.Dataset):
  ...
  def random_rotate(self, xyzr, r_angle=360, is_random=True, add_noise = True, rand_tr = False):
    ...
    if add_noise:
      ...
      noise = np.clip(n_sigma * np.random.randn(*augmented_scan.shape), -0.03, 0.03)
  ...
  def pnv_preprocessing(self, xyzr, l=25):
    ...
    ind = np.argwhere(xyzr[:, 2] >= -l).reshape(-1)
    xyzr = xyzr[ind]   
    # visualize_scan_open3d(xyzr[:,:3])   

    vox_sz = 0.3
    # ds_count = 0
    while len(xyzr) > 4096:
      # print(ds_count, len(xyzr))
...
class CollationFunctionFactory:
  ...
  def collate_tuple(self, list_data):
    ...
    for batch_data in list_data: 
      ...
      # contrastive_tuple = torch.FloatTensor(contrastive_tuple)
      # outputs.append(contrastive_tuple)
