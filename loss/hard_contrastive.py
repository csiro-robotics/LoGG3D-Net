...
def best_pos_distance(query, pos_vecs):
    ...
    diff = ((pos_vecs - query_copies)**2).sum(1)#((pos_vecs - query_copies) ** 2).sum(2)
    max_pos_dist = diff.max()
    return max_pos_dist

def triplet_loss(tuple_vecs, config):
    tuple_vecs = torch.split(tuple_vecs, [1,config.positives_per_query, config.negatives_per_query])
